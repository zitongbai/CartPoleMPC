...
simulink_opts.inputs.reset_solver = 1;
    % Determines if the solver's iterate is set to all zeros 
...
x0 = [1.5; deg2rad(180); 0; 0]; % initial state

%% model dynamics
model = acadosCartPoleModel();
...
%% Create S function
cd c_generated_code
make_sfun;  % compile s function for OCP solver
cd ..
